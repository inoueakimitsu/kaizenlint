...
ユーザー入力や信頼できないデータには、処理前の状態を示す属性を名前に追加します。CLI ツールの argparse 引数や環境変数など、ユーザー自身が直接制御する入力は、このルールの対象外です。このルールは Web アプリケーションの HTTP リクエスト パラメーター、外部 API のレスポンス、ファイルから読み込んだデータなど、第三者が制御可能な入力に適用してください。
...
ただし、肯定系にするためだけに、一時変数を作るのはやりすぎのため避けてください。
