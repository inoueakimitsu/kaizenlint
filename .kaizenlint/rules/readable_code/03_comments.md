...
定数を定義する際は、その値がなぜその値に設定されているのかを説明するコメントを記述します。経験的に決めた閾値の場合は「端末出力に適した妥当な上限値」のような簡潔な説明で十分です。
...
他のプログラマが間違って関数を使用する可能性がある場合や、実装に非自明な制約や副作用がある場合は、事前に警告コメントを記述します。すべての外部呼び出しに警告を追加する必要はありません。特に非自明な副作用 (例: ハングの可能性、データ損失のリスク) がある場合にのみ適用してください。
...
コメントは情報密度が高く簡潔でなければなりません。複数行で説明できることは 1 行に集約すべきです。ただし docstring の Parameters / Returns セクションは、各パラメーターの説明のために複数行になることが許容されます。簡潔にすべきなのは概要説明部分です。
